...
import { getTimesheetApplicationCollection } from '@root/database'
...
export default async function(req, res) {
  ...
  if (permissionToEdit || role === "admin" || role === "editor") {
    const newApprovalUser = await initNewApprovalUser(userId)
    const updateTimesheetApp = {
      status: isApproved ? 1 : 0,
      updated: new Date(),
      approvalUsers: firebase.firestore.FieldValue.arrayUnion(newApprovalUser)
    }
    await getTimesheetApplicationCollection().doc(timesheetAppId).update(updateTimesheetApp)
